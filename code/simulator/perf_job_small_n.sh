...
mkdir -p test_results/seq_optimized
cd build 
...
for N in $N_VALUES; do
    ...
    srun perf stat -e cycles,instructions,cache-references,cache-misses,L1-dcache-load-misses \
        -o test_results/seq_optimized/perf_N$N.txt ./build/simulate $N 3600 36000 > simulation_results/baseline/result_N$N.txt

    # Trials 2-10: Capture clean wall-clock time
    echo "Running remaining trials for timing..."
    for i in $(seq 2 $TRIALS); do
       { time ./build/simulate $N 3600 36000 ; } 2>> test_results/seq_optimized/time_results_$N.txt > /dev/null
    done
done
